...
def adj_matrix2():
  ...
  #### matrix is modified to better represent regulatory effect
  adj[adj == 1] = -1
  adj[adj==0]=1
  print('data loaded')
  ...
  adj=adj[y_ind1,:]
  mRNA=np.exp2(mRNA)-.001       #### converted back into raw value
...
######network propagation
def NP(mRNA,miRNA,alpha,maxiter,adj):  
...
##### correlation
def corr(protein,sample_name,mRNA_name,ROC):
  ...
  print('gene_exp calculated')
  print('dimension of gene_exp is: ', np.shape(gene_exp))
  ...
  for i,x in enumerate(uni_gene):   
    ...
    if c.size == 0:
      ...
      j+=1
      continue
    gene_matched_spectral_count.append(spectral_count[c,4:][0])
    k+=1
  ...
  final_protein=[]
  d=[]
  for i,x in enumerate(spectral_count[0,4:]):

    if 'POOL Spectral Counts' in x:
      names=x.split(':')[0:3] 
...
adj,mRNA,miRNA,sample_name,uni_mRNA_names=adj_matrix2()
...
for x in alpha:
  protein=NP(mRNA,miRNA,x,1000,adj)
  pear_coef,spearman_coef,roc=corr(protein,sample_name,uni_mRNA_names,ROC)
...
#np.savetxt('pearson_coef_1.csv',pearson, delimiter=',')
#np.savetxt('spearman_coef_1.csv',spearman, delimiter=',')
#np.savetxt('ROC_1.csv',roc12, delimiter=',',fmt='%1.1d')
