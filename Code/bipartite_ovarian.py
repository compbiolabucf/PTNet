...
def corr(protein,sample_name,mRNA_name,ROC):
  ...
  for i,x in enumerate(uni_gene):   
    ...
    if c.size == 0:
      ...
      j+=1
      continue
    gene_matched_spectral_count.append(spectral_count[c,:-11][0])
    k+=1
  ...
  final_protein=[]
  d=[]
  for i,x in enumerate(spectral_count[0,:-11]):
...
for x in alpha:
  protein=NP(mRNA,miRNA,adj,x,1000)
